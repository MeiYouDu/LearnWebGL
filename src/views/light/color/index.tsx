...
function getGLFromScene(scene: Scene): WebGL2RenderingContext | null {
	const maybe = (
		scene as unknown as {
			gl?: {
...
export default function BoxSceneReact() {
	...
	useEffect(() => {
		...
		const lightPos = vec3.fromValues(sin(angle) * 2, cos(angle) * 2, -3);
		...
		const boxGeometry = new Geometry({
			...
			uniformsSetter(glInner: WebGL2RenderingContext, shaderInner: Shader) {
				shaderInner.setVec4(vec4.fromValues(1, 1, 1, 1.0), "lightColor");
				shaderInner.setVec3(lightPos, "lightPos");
				shaderInner.setVec3((scene as Scene).camera.position, "cameraPos");
			},
		});
		...
		// register into scene
		(scene as Scene).geometryMap.set(boxGeometryInstance, boxGeometryInstance);
